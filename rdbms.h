...
int main(){
	...
	temp2.push_back(conjN);
	ConditionNode* condition = new ConditionNode(temp2);

	db.allTables["artists"].printTable();

	vector<vector<Datum> >::iterator it;
	it = db.allTables["artists"].data.end();
	while (it != db.allTables["artists"].data.begin())
	{
		it--;
		cout << "eval result: " << condition->eval(db.allTables["artists"].attributeNames, *it) << "\n";
	}
